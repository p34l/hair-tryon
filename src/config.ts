...
export const PRESETS: ColorPreset[] = (() => {
  ...
  // ТЕСТ ПРИЧИНЫ ЛАГА: оставляем ОДИН цвет (берём насыщенный из середины), чтобы
  // в палитре грузилась ~1 картинка вместо ~150. Если лаг исчезнет — виноваты
  // кольоры/картинки, а не MediaPipe. Вернуть: убрать .slice ниже.
  return out.slice(Math.floor(out.length / 2), Math.floor(out.length / 2) + 1);
})();
