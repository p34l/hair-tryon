...
export async function startCamera(video: HTMLVideoElement): Promise<CameraHandle> {
  ...
  const stream = await navigator.mediaDevices.getUserMedia({
    ...
    video: {
      facingMode: 'user',
      // 720p достаточно: рендер-таргет всё равно кэпится (см. renderer), а 1080p
      // только грузит декодер/загрузку текстуры без выигрыша в картинке.
      // Инференс всё равно на 256 (бриф соблюдён).
      width: { ideal: 1280 },
      height: { ideal: 720 },
      // 30 к/с — стабильнее и дешевле; rVFC привязан к реальной частоте кадров.
      frameRate: { ideal: 30 },
    },
  });
