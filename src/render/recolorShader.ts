...
void main() {
  ...
  if (u_lutRow >= 0.0) {
    ...
    float nl = oLab.x + (tLab.x - 0.5) * u_lumaShift;
    // Мягкое «плечо» вместо жёсткого clamp в белый: светлые волосы под светлый
    // оттенок (платина/блонд) НЕ выбиваются в плоское белое пятно — блики
    // сохраняют градиент и текстуру (натуральнее, без эффекта «вылитого» цвета).
    float knee = 0.82;
    if (nl > knee) nl = knee + (1.0 - knee) * (1.0 - exp(-(nl - knee) / (1.0 - knee)));
    float newL = clamp(nl, 0.0, 1.0);
    ...
    // Усиливаем по хроме ТОЛЬКО очень бледные цели (платина/серебро, tC<~0.06):
    // они иначе выходят почти белыми. Блонд (tC выше) уже тёплый — его НЕ трогаем,
    // иначе становится кислотно-жёлтым и неестественным.
    chroma *= mix(1.35, 1.0, smoothstep(0.03, 0.085, tC));
    // Десатурацию бликов делаем ТОЛЬКО для насыщенных целей (защита от неона).
    // Для светлых/блонд оттенков (малая tC) её НЕ применяем — иначе блики сереют
    // и блонд теряет тёплый тон, выглядит неестественно «вылинявшим».
    float satTarget = smoothstep(0.04, 0.12, tC);
    chroma *= mix(1.0, mix(1.0, 0.7, smoothstep(0.78, 1.0, oLab.x)), satTarget);
